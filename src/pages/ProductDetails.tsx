...
const ProductDetails = () => {
  ...
  const { service, productName } = useMemo(() => {
    const svc = getServiceBySlug(slug || "");
    if (!svc) return { service: undefined, productName: undefined };
    const name = svc.products.find((p) => toSlug(p) === (product || ""));
    return { service: svc, productName: name };
  }, [slug, product]);

  useEffect(() => {
    if (!service || !productName) return;
    const title = `Udaan | ${productName} – ${service.title}`;
    const description = `${productName} in ${service.title}. Specifications, high-quality images, and export options by Udaan.`;
    ...
    canonical.setAttribute("href", `/services/${service.slug}/${toSlug(productName)}`);
    ...
    const ldJson = {
      ...
      image: categoryImageMap[service.slug],
      description,
    };
